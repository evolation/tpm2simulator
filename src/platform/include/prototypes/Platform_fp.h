...
// This function sets current system clock time as t0 for counting TPM time. This function is
// called at a
...
// Get the most recent command locality in locality value form. This is an integer value for locality and
// not a
...
// This function is used by the simulator to set the error flags in the NV subsystem to simulate an error in
// the
...
// This function is used to update NV memory. The write is to a memory copy of NV. At the
// end of the

// current command, any changes are written to the actual NV memory.

// NOTE: A useful optimization would be for this code to compare the current contents of NV with the local
// copy and note
...
// Function is used to set a range of NV memory bytes to an implementation-dependent value.
// The value
...
// Function: Move a chunk of NV memory from source to destination This function should
// ensure that if
...
// This version of RunCommand() will set up a jum_buf and call ExecuteCommand(). If the
// command
...
// NOTE: If a platform unique value has unequal distribution of uniqueness and bSize is smaller than the
// size of the
