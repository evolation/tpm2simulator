...
// implementation is a UINT64 but can be smaller. The "tracking array" (contextArray) only has
// 16-bits per
...
// assigned if it would be the same as an existing value. Since the array holds 16 bits, after a
// context has
// been saved, an additional 2^16-1 contexts may be saved before the count would again match.
// The
// normal expectation is that the context will be flushed before its count value is needed again but it is
// always possible to have long-lived sessions. The contextID is assigned when the
// context is saved
...
// by the TRM is to load the oldest saved session context (the one found by the TPM), and save it.
// Since
...
// several contexts in order to save an additional one. This should happen very infrequently. When the
// TPM
// searches contextArray and finds that none of the contextIDs match the low-order 16-bits of contextCount,
// the TPM can copy the low bits to the contextArray associated with the session,
// and increment
...
// to create the
// zero-based slot number. There is one significant corner case in this scheme. When the contextCount is
// equal
// to a value in the contextArray, the oldest session needs to be recycled or flushed. In order to recycle the
// session, it must be loaded. To be loaded, there must be an available slot. Rather than require that a spare
// slot
// be available all the time, the TPM will check to see if the contextCount is equal to some value
// in the
...
// active sessions rather than a list of contextID). The full contextID has high-order bits that are either the
// same
// as the current contextCount or one less. It is one less if the 16-bits of the contextArray has a value that
// is larger
