...
// This function performs the RSAEP operation defined in PKCS#1v2.1. It is an
...
)
{
    ...
    TPM2B_4BYTES(e) = {{
            4, {
                (BYTE)((RSA_DEFAULT_PUBLIC_EXPONENT >> 24) & 0xff),
...
// This function performs OAEP padding. The size of the buffer to receive the OAEP padded
// data must
...
// This function creates an encoded block of data that is the size of
// modulus. The function uses the
...
// This function is used by TPM2_RSA_Decrypt() and TPM2_RSA_Encrypt(). It
// sets up the rules to select a
...
// NOTE: If dIn has fewer bytes than cOut, then we don't add low-order zeros to dIn to make it the size of
// the RSA key for
