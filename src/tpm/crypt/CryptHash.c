...
// This function returns a pointer to the hash info for the algorithm. If the algorithm is not supported,
// function
...
// This function is used to iterate through the hashes. TPM_ALG_NULL is returned for all
// indexes that are

// not valid hashes. If the TPM implements 3 hashes, then an index value of 0
// will return the first

// implemented hash and an index of 2 will return the last. All other index
// values will return
...
// Returns the size of the digest produced by the hash. If hashAlg is not a hash algorithm,
// the TPM will
...
// This function is used to import the hash state. This function would be called to import a hash state
// when
...
// This function is used to include an integer value to a hash stack. The function marshals the integer into
// its
...
// This function is used to start an HMAC using a temp hash context. The function does the initialization
// of
...
// This function is called to complete an HMAC. It will finish the current digest, and
// start a new digest. It will

// then add the oPadKey and the completed digest and return the results in dOut. It
...
// The caller must provide a block of memory in which the hash sequence state is kept. The
// caller should
...
// function returns the length of the mask produced which could be zero if the digest
// algorithm is not
...
// The Z and keyStream pointers are not allowed to be NULL. The other pointer values may
// be NULL. The

// value of sizeInBits must be no larger than (2^18)-1 = 256K bits (32385 bytes). Any error in
// the processing
