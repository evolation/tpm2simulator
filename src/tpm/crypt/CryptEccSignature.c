...
// Function to adjust the digest so that it is no larger than the order of the curve. This is used for
// ECDSA
...
// ECSCHNORR making for a lot of confusion. In particular, the k value in this function is
// value in this
...
// Function to reduce a hash result if it's magnitude is to large. The size of number is set so
// that it has no

// more bytes of significance than the reference value. If the resulting number can
// have more bits of
...
// This function validates an ECDSA signature. rIn and sIn should have been
...
// If B or M is provided, they must be on the curve defined by curveId. This routine does
// not check that they

// are on the curve and results are unpredictable if they are not.

// It is a fatal error if r is NULL. If B is not NULL, then it is a fatal error if d is NULL or
// if K and L are both
