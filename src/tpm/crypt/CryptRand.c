...
// secure application so that both reside inside the same boundary [SP 800-90A 8.5]. The
// secure
...
// This function does the incremental update of the derivation function state. It encrypts the
// iv value and

// XOR's the results into each of the blocks of the output. This is equivalent to processing all
// of input data
...
// This initializes the output blocks with an encrypted counter value and initializes the
// key schedule.
static void
DfStart(
    PDF_STATE dfState,
...
// This function is called to get the result of the derivation function computation. If the buffer is not
// full, it is

// padded with zeros. The output buffer is structured to be the same as a DRBG_SEED value so
// that the
...
// Function to take an input buffer and do the derivation function to produce a DRBG_SEED
// value that can
...
// operation, the counter is incremented before each block is encrypted. This
// function implements an
...
// The functions in this section are the interface to the RNG. These are the functions
// that are used by
...
// Function to reseed the DRBG with additional entropy. This is normally called before
// computing the
...
// example, if max == 4 and min == 2, then the number will be between 0x0010 and 0x1111 inclusively.
// If
