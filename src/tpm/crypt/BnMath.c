...
// This functions in this file perform simple operations on these big numbers. Only the more
// complex
...
// NOTE: There is an intention to continue to augment these functions so that there would be no need to
// use an external
...
// made because our only bigNum computations are on large positive numbers (primes) or
// on fields.
...
// Compare a bigNum to a crypt_uword_t. -1 op1 is less that word 0 op1 is
// equal to word 1 op1 is greater
...
// Returns the bit number of the most significant bit of a crypt_uword_t. The number
...
// Check to see if a bit is SET in a bignum_t. The 0th bit is the LSb() of d[0] If a bit is
...
// Function to mask off high order bits of a big number. The returned value will have
// no more than maskBit
